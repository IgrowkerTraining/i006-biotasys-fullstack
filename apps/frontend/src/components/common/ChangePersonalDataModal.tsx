...
interface ChangePersonalDataModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (data: { fullName: string; password: string }) => Promise<void>;
  title?: string;
  subtitle?: string;
...
export const ChangePersonalDataModal: React.FC<ChangePersonalDataModalProps> = ({
  open,
  onClose,
  onConfirm,
  title = "Cambiar datos personales",
  subtitle = "Introduzca un nuevo nombre y apellidos y su contraseña existente",
  currentFullName = "",
  isLoading = false,
}) => {
  ...
  const validateFullName = useCallback((value: string) => {
    if (!value.trim()) {
      setErrors(prev => ({ ...prev, fullName: "El nombre no puede estar vacío" }));
      return false;
    }
    if (value.trim().length < 3) {
      setErrors(prev => ({ ...prev, fullName: "El nombre debe tener al menos 3 caracteres" }));
      return false;
    }
  ...
  const handleConfirm = async () => {
    ...
    if (!isFullNameValid || !passwordValid) {
      return;
    }
  ...
  const isFormValid = fullName.trim() !== "" && passwordValid && !isLoading;
