...
interface ChangeLaboratoryModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (data: { laboratory: string; password: string }) => Promise<void>;
  title?: string;
  subtitle?: string;
...
export const ChangeLaboratoryModal: React.FC<ChangeLaboratoryModalProps> = ({
  open,
  onClose,
  onConfirm,
  title = "Cambiar centro adscrito",
  subtitle = "Introduzca un nuevo centro adscrito y su contraseña existente",
  currentLaboratory = "",
  isLoading = false,
}) => {
  const [laboratory, setLaboratory] = useState(currentLaboratory);
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [passwordValid, setPasswordValid] = useState(false);
  const [errors, setErrors] = useState<{ laboratory?: string; password?: string }>({});
  const [checkingPassword, setCheckingPassword] = useState(false);

  // Validar centro
  const validateLaboratory = useCallback((value: string) => {
    if (!value.trim()) {
      setErrors(prev => ({ ...prev, laboratory: "El centro no puede estar vacío" }));
      return false;
    }
    if (value.trim().length < 2) {
      setErrors(prev => ({ ...prev, laboratory: "El centro debe tener al menos 2 caracteres" }));
      return false;
    }
    setErrors(prev => ({ ...prev, laboratory: undefined }));
    return true;
  }, []);
  ...
  const handleConfirm = async () => {
    const isLaboratoryValid = validateLaboratory(laboratory);
    
    if (!isLaboratoryValid || !passwordValid) {
      return;
    }
  ...
  const isFormValid = laboratory.trim() !== "" && passwordValid && !isLoading;
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="ej: BiomeLab, Hospital Central"
            value={laboratory}
            onChange={(e) => setLaboratory(e.target.value)}
            onBlur={() => validateLaboratory(laboratory)}
